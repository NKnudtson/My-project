...
namespace Core.Camera.Editor
{
	...
	public class CameraRigEditor : UnityEditor.Editor
	{
		...
		void OnSceneGUI()
		{
			...
			var fmh_42_60_638011884439706990 = Quaternion.LookRotation(Vector3.up); Vector3 bottom = Handles.FreeMoveHandle(bottomPosition, size, snap,
			                                        Handles.RectangleHandleCap);
			var fmh_44_54_638011884439729725 = Quaternion.LookRotation(Vector3.up); Vector3 top = Handles.FreeMoveHandle(topPosition, size, snap,
			                                     Handles.RectangleHandleCap);
			var fmh_46_56_638011884439732105 = Quaternion.LookRotation(Vector3.up); Vector3 left = Handles.FreeMoveHandle(leftPosition, size, snap,
			                                      Handles.RectangleHandleCap);
			var fmh_48_58_638011884439734269 = Quaternion.LookRotation(Vector3.up); Vector3 right = Handles.FreeMoveHandle(rightPosition, size, snap,
			                                       Handles.RectangleHandleCap);
